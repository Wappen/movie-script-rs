...
fn main() {
    ...
    let mut watchable_movies: Vec<(movie::Movie, i32)> = movies.into_iter()
        .filter(|(_, fsk)| age >= *fsk).collect();

    // Sort alphabetically by title
    watchable_movies.sort_by(|(m1, _), (m2, _)|
        m1.title.partial_cmp(&m2.title).unwrap());
    ...
    println!();
    let index: usize = {
        let index_pred = |index: &usize| {
            *index > 0 && *index <= watchable_movies.len()
        };
        let on_error = |e: &InputError<usize>| {
            match e {
        ...
        get_input("Select a movie by typing in it's number: ", index_pred, on_error)
    };
